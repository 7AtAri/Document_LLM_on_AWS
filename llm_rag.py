...
import time
import os
from config import S3_BUCKET_NAME, S3_MODEL_PATH, LOCAL_MODEL_PATH, DOCS_FOLDER, TASK, S3_DOCS_FOLDER, QUESTION
...
def main(s3_bucket_name, s3_model_path, s3_docs_folder, 
                local_model_path, local_docs_folder, 
                task, question):
    ...
    if os.path.exists("vector_db") and os.listdir("vector_db"):
        ...
        embedding_vectors = preprocess_documents(documents, save_preprocessed=True)
...
if __name__ == "__main__":
    main(S3_BUCKET_NAME, S3_MODEL_PATH, S3_DOCS_FOLDER, 
                LOCAL_MODEL_PATH, DOCS_FOLDER, 
                TASK, QUESTION)
